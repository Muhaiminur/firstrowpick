...
class MlsGamesList extends StatelessWidget {
  const MlsGamesList({
    super.key,
  ...
  Widget build(BuildContext context) {
    return GetBuilder<EventController>(builder: (controller) {
      return controller.mlsEventloading.value
          ? controller.mlsgamedata.isNotEmpty
              ? ListView.separated(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return ScoreItem(
                      item: controller.mlsgamedata.elementAt(index),
                    );
                  },
                  itemCount: controller.mlsgamedata.length,
                )
              : Center(
                  heightFactor: 3,
                  child: Text(
