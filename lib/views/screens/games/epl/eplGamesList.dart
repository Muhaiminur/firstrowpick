...
class EplGamesList extends StatelessWidget {
  const EplGamesList({
    super.key,
  ...
  Widget build(BuildContext context) {
    return GetBuilder<EventController>(builder: (controller) {
      return controller.eplEventloading.value
          ? controller.eplgamedata.isNotEmpty
              ? ListView.separated(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return InkWell(
                        child: ScoreItem(
                      item: controller.eplgamedata.elementAt(index),
                    ));
                  },
                  itemCount: controller.eplgamedata.length,
                )
              : Center(
                  heightFactor: 3,
                  child: Text(
